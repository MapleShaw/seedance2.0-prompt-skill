...
3. **图片驱动（路径 C）**：用户丢图进来想做视频（尤其打满 15s 做社交媒体引流）。不是"平均让画面动起来"，而是**从图里识别出一个最有戏的点极致放大**。
...
**路径C — 图片驱动（Image-Driven）：**
- 触发：用户上传/粘贴/拖入图片，或要求"用这张图做视频"、"图生视频提示词"、"根据图片生成视频"、"这张图怎么做成视频"、"打满 15s 的社交媒体爆款"
- 核心任务：**从图里识别出一个最有戏的点极致放大**，设计 15s 演化轨迹
- **必须走两轮交互**：
  1. 第 1 轮：读图分析（主体/氛围/最有戏元素/比例诊断/风险检查） + 2-3 个 Hook 候选模式
  2. 第 2 轮：用户选定模式后，输出完整可复制的 Seedance 提示词
- **可选 Step 4（CLI 执行）**：第 2 轮提示词输出后，**询问**用户是否直接调用 `dreamina` CLI 生成视频；默认不跑，用户回 `跑` / `yes` 才执行。默认配置 `seedance2.0_vip` + `720p` + `15s`。详见 [cli-integration.md](references/cli-integration.md)
- 默认目标：打满 15 秒，社交媒体引流
...
**第 2 轮输出**（选定后的完整提示词）：按"完整模式"格式，但额外标注 **Hook 模式** 和 **图的角色**，并在末尾给"设计意图"说明前 2 秒生死线、核心 hook、二刷钩子、传播预期。
