...
- 路径 D（分镜板驱动，v0.1 试运行）：起点是 "N 宫格分镜板"，做的是多格拆解+串联（详见 `experiments/proposals/path-D-storyboard-driven-draft.md`）
...
不适合 D-1 叙事拼接时的备选方案：
- **方案 A（首选·已升级为 D-3「记忆/心境蒙太奇法」）**：放弃硬时序框架，用**情绪母题黏合剂 + 语义转场流 + 首尾呼应**把差异片段串成"回忆 MV"。母题让"硬切的违和"转化为"回忆的跳跃感"——这是独立场景型多宫格能跑通的根本。完整模板见 `experiments/proposals/path-D-storyboard-driven-draft.md` §1.4
...
- **路径 D 分镜板驱动**（试运行）→ `experiments/proposals/path-D-storyboard-driven-draft.md`
