...
> **未来 Agent 写女性 / 室内 / 多人场景 prompt 前必读。完整推导见 `handoff/results/summary.md` 第 20 节**。
...
- 完整翻车历程 + 真因定位过程：`handoff/results/summary.md` **第 20 节**
- 完整四层敏感词清单：本文档 **下方"版权/商标/地缘敏感词规避清单"小节**（含五关扫描脚本）
- 案例参考（11 版翻盘）：`handoff/results/cases/case-17-公关经理介绍排成一排.md`
...
- 路径 D（分镜板驱动，v0.1 试运行）：起点是 "N 宫格分镜板"，做的是多格拆解+串联（详见 `handoff/proposals/path-D-storyboard-driven-draft.md`）
...
> 前 8 条是 v3 沉淀的实战原则（详见 `handoff/results/summary.md` 第 4 节）。
...
不适合 D-1 叙事拼接时的备选方案：
- **方案 A（首选·已升级为 D-3「记忆/心境蒙太奇法」）**：放弃硬时序框架，用**情绪母题黏合剂 + 语义转场流 + 首尾呼应**把差异片段串成"回忆 MV"。母题让"硬切的违和"转化为"回忆的跳跃感"——这是独立场景型多宫格能跑通的根本。完整模板见 `handoff/proposals/path-D-storyboard-driven-draft.md` §1.4
...
" handoff/results/cases/case-XX-xxx.md
```
...
| 场景类型 | Hook 范式 | 文件位置 |
|---------|---------|---------|
| 演唱会前排追星 | 凝视模式 | `handoff/results/cases/case-01-演唱会.md` 第 15 节 v5（1456 字符） |
| 多人场景合影前打闹 | 凝视模式 + 跨人物互动 | `handoff/results/cases/case-03-化妆间.md` 第 15 节 v5（1701 字符） |
| 课堂偷拍 | 凝视模式 + 时间分段 | `handoff/results/cases/case-04-讲课.md` 第 15 节 v5（1886 字符） |
| 直播流偷拍看球 | 凝视模式（奠基 case） | `handoff/results/cases/case-05-球场大屏.md` 第 15 节 v4.1（1408 字符） |
...
- **方法论 v3 完整推导** → `handoff/results/summary.md`
- **路径 D 分镜板驱动**（试运行）→ `handoff/proposals/path-D-storyboard-driven-draft.md`
